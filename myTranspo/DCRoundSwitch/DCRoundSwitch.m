...
- (void)dealloc
{
	...
	[onText release];
	[offText release];
    if(iconText != nil) [iconText release];
...
- (void)setIconText:(NSString *)newIconText
{
    if(iconText != newIconText)
    {
        [iconText release];
        iconText = [newIconText copy];
        knobLayer.knobIcon = [UIImage imageNamed:iconText];
        [knobLayer setNeedsDisplay];
    }
}
