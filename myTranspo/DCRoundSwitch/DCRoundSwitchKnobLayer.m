...
- (void)drawInContext:(CGContextRef)context
{
    ...
    if(_knobIcon != nil)
    {
        CGRect knobIconRect = CGRectZero;
        knobIconRect.origin.x = (knobRect.size.width / 2) - (_knobIcon.size.width / 2) + knobRect.origin.x;
        knobIconRect.origin.y = (knobRect.size.height / 2) - (_knobIcon.size.height / 2) + knobRect.origin.y;
        knobIconRect.size = _knobIcon.size;
        
        CGContextRestoreGState(context);
        
        UIGraphicsBeginImageContext(_knobIcon.size);        
        UIGraphicsPushContext(context);
        
        [_knobIcon drawInRect:knobIconRect];
        /*
        CGContextTranslateCTM(context, 0, knobIcon.size.height);
        CGContextScaleCTM(context, 1.0, -1.0);
        CGContextDrawImage(context, knobIconRect, knobIcon.CGImage);        
        */
